export async function processAudioBuffer(
  ...
  if (audioBuffer.numberOfChannels >= 2) {
    ...
    normalizedChannelData = leftChannelData.map((sample, index) => {
      ...
      const combinedSample = Math.round(
        (sample / 2 + rightChannelData[index] / 2) * 100
      );
  ...
  return normalize(
    sliceAverage(Array.from(normalizedChannelData), slices),
    0.1,
    1
  );
  ...
  function normalize(array: number[], min: number, max: number): number[] {
    ...
    return array.map(
      (val) => ((val - lowest) / (highest - lowest)) * (max - min) + min
    );
  }
