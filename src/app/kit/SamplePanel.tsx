...
export const SamplePanel = ({ ...props }) => {
  ...
  return (
    <div className={`flex items-center p-2 gap-2 border-2 bg-base-300/50 active:bg-base-300/30 border-primary rounded-lg  backdrop-blur-sm m-0.5 ring-0 hover:ring-2 ring-primary hover:ring-primary-focus  transition hover:border-primary-focus  ${props.isDragging ? 'active:drop-shadow-hard active:-translate-x-2 active:backdrop-blur-md active:-translate-y-2 active:border-primary-content' : ''} `}>
      {isAudioPlaying() !== true && <IconButton
      ...
      <p className="grow truncate text-base-content select-none">{props.sampleName}</p>
      <WaveFormDisplay waveform={sampleList[props.index].waveformCache} progress={playProgress} height={40} audioPlaying={isAudioPlaying()}/>
